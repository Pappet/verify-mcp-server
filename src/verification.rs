...
async fn run_check(check: &Check, input: Option<&str>) -> RawResult {
    match &check.check_type {
        CheckType::CommandSucceeds {
            ...
            working_dir,
            timeout_secs,
        } => run_command_succeeds(command, working_dir.as_deref(), *timeout_secs).await,
        ...
        CheckType::CommandOutputMatches {
            ...
            run_command_output_matches(command, pattern, working_dir.as_deref(), *timeout_secs)
                .await
        }
...
async fn run_command_succeeds(
    ...
    match execute_command(command, working_dir, timeout_secs).await {
...
"#
    ...
    match execute_command(&command, working_dir, 60).await {
