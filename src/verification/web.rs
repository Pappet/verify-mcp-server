...
use crate::verification::helpers::{
    add_working_dir_hint_if_needed, resolve_path, shell_escape, truncate,
};
...
pub(crate) async fn check_jest_vitest_result(
    ...
    match execute_command(&cmd_str, working_dir, timeout_secs, None, true).await {
        Ok((code, stdout, _stderr)) => {
            ...
            let json_str = if start_idx < end_idx {
                &stdout[start_idx..end_idx]
            } else {
                ""
            };
...
pub(crate) fn check_css_html_consistency(
    ...
    if missing.is_empty() {
        ...
        RawResult {
            status: CheckStatus::Failed,
            message: format!(
                "{} of {} HTML classes NOT found in CSS",
                missing.len(),
                html_classes.len()
            ),
