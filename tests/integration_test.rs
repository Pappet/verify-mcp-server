...
async fn recv_msg(
    ...
    loop {
        let line = reader
            ...
            .await
            .unwrap()
            .expect("Unexpected EOF while waiting for response");
...
async fn test_json_rpc_integration() {
    ...
    assert!(
        init_resp["result"].get("protocolVersion").is_some(),
        "Expected protocolVersion in initialize response"
    );
    assert!(
        init_resp["result"].get("serverInfo").is_some(),
        "Expected serverInfo in initialize response"
    );
    assert!(
        init_resp["result"].get("capabilities").is_some(),
        "Expected capabilities in initialize response"
    );
    ...
    let content = create_resp["result"]["content"]
        .as_array()
    ...
    let create_result_json: serde_json::Value = serde_json::from_str(text).unwrap_or_else(|e| {
        panic!(
            "Contract result text should be JSON. Raw text was: '{}', Error: {}",
            text, e
        )
    });
    let contract_id = create_result_json["contract_id"]
        .as_str()
        .expect("Expected contract_id in create response");
    ...
    let lcontent = list_contracts_resp["result"]["content"]
        .as_array()
        .expect("Expected content array");
    ...
    let contracts = ljson["contracts"]
        .as_array()
        .expect("Expected contracts array");
    assert!(
        contracts.iter().any(|c| c["id"] == contract_id
            || c["contract_id"] == contract_id
            || c["uuid"] == contract_id),
        "Listed contracts should contain the newly created contract_id. Contracts list: {:?}",
        contracts
    );
    ...
    let dcontent = del_resp["result"]["content"]
        .as_array()
        .expect("Expected content array");
    let dtext = dcontent[0]["text"].as_str().unwrap();
    assert!(
        dtext.contains("deleted") || dtext.contains("Deleted"),
    ...
    assert_eq!(
        parse_err_resp["error"]["code"], -32700,
    ...
    assert!(
        status.success(),
        "Server should exit cleanly with success on stdin close"
    );
}
